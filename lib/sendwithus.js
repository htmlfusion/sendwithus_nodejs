...
Sendwithus.prototype.createCommand = function(command, data, callback){
    return {
        path: '/api/' + 'v' + API_VERSION + '/' + command,
        method: 'POST',
        body: data
    };
}

Sendwithus.prototype.batch = function(commands, callback) {
    var url = this._buildUrl('batch');

    var options = this._getOptions();

    var that = this;
    this.emit('request', 'POST', url, options.headers, commands);
    restler
        .postJson(url, commands, options)
